...
import com.github.angerona.fw.gui.util.tree.UserObjectWrapperController;
import com.github.angerona.fw.gui.util.tree.UserObjectWrapperTreeCellRenderer;
...
public class ReportTreeView
	...
	private static class EntryUserObjectWrapper extends DefaultUserObjectWrapper {
		...
		private static Logger LOG = LoggerFactory.getLogger(EntryUserObjectWrapper.class);
		...
		public void onActivated() {
			...
			LOG.info(AdditionalData.DEBUG_MARKER, "Активация агента через специальный враппер");

			if(entry.getAttachment() != null) {

				Entity at = entry.getAttachment();
				LOG.info(AdditionalData.DEBUG_MARKER, "Сущность которая приклеплена к враперу " + at);
